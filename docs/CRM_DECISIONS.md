...
## One APK, role-aware landing (/launch)
The Captain APK is ONE app for ALL staff — login role + Page Access decide what each user sees (no separate GRE app). The APK/PWA now open `/launch` (build.gradle launchUrl + twa-manifest startUrl + public/captain.webmanifest start_url; webmanifest `id` stays "/captain" so installed PWAs aren't orphaned; scope "/" already covers it). `/launch` (src/app/launch/page.tsx, server component, dynamic) → getCurrentUser → redirect(homePathFor(user)); not signed in → /login?next=/launch. Login default `next` is now `/launch`. `homePathFor` (src/lib/page-catalog.ts) returns the first PREFERRED candidate the user canAccessPage(): management→'/', then /captain, /crm-calls/recovery, /crm-calls, /dine-in/kitchen, /dine-in/floor, /cashier, /requisitions, /tasks/my, '/'; fallback firstAllowedPath. '/launch' added to ALWAYS_ALLOWED so the proxy lets every role reach the resolver. Pure routing — never grants access. Unit-tested matrix (admin→/, GRE→recovery, captain→POS, kitchen→KDS, none→/login).

## Exact callback durations — Captain APK call-log bridge (Android)
The Captain APK (Bubblewrap TWA, `com.akanhyd.fnb.captainapp`, captain-android/) v1.1.0 adds `CallLoggerActivity` (scheme `akancall://log?phone=&recovery=&ret=`): places the call (ACTION_CALL if CALL_PHONE granted, else ACTION_DIAL), then reads the EXACT duration from the device CallLog (READ_CALL_LOG, polled ~12s after resume, digit-suffix number match) and deep-links back `https://fnb.akanhyd.com<ret>?cb=1&cb_phone&cb_duration&cb_connected&cb_src=calllog|approx&cb_recovery` — reopens in-app (verified TWA link). Auth never leaves the web session. Web side: `src/lib/ct/twa.ts` (TWA detection via android-app:// referrer flag persisted in localStorage + cb=1 proof; intent:// URL builder with S.browser_fallback_url), CallbackButton uses the intent bridge when `isCaptainApp()` (else tel:+timer), and on mount the MATCHING row claims cb_* params (recovery id first, else phone-suffix) → prefilled sheet with green "Exact — read from the phone's call log" badge; params stripped via replaceState (claim = strip, no module flag). log-callback accepts `source: call_log|approx|timer|manual` (attempt + raw_payload). Denied permission/OEM quirk → cb_src=approx wall-time fallback. iOS/PWA unchanged (timer flow).
...
## Mobile-collapsing toolbar (2026-07-20)
`src/components/ct/CollapsibleToolbar.tsx` — reusable wrapper: on `<md` the wrapped filter/tool cluster collapses behind a "Filters & options" button (SlidersHorizontal + count badge + chevron; auto-opens when `activeCount>0` so an active filter is never hidden); on `md+` the cluster renders inline unchanged (button `md:hidden`, panel `md:block`). Purely presentational — pages keep owning filter state. Wired into the three filter-heavy pages, keeping the primary **search box** and **status tabs / summary chips** visible above the dropdown:
- `guests/page.tsx` — collapses badge/tag/converted/date-range/clear + the mobile sort control; `activeCount` = # of set filters (search excluded).
- `log/page.tsx` — collapses status/agent/date-range/clear (summary chips stay visible); `activeCount` = status+agent+from+to.
- `bookings/page.tsx` — collapses the date-range block (status tabs stay visible); label "Date range"; `activeCount` = 1 when the window differs from the default today→+7d.
Recovery (tabs+search only), Dashboard (7d/30d toggle), Live (no filter row), Settings (config sections) left as-is — already light. Verified on-screen at 375px (collapsed by default, data immediately visible; expands to reveal controls) and desktop (inline, no button). tsc clean, prod build ok.
