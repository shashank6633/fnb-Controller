...
// ──────────────────────────────────────────────────────────
function Landing({ onEnter, table }) {
  return (
    <div style={{
        ...
        <>
          ...
          <div style={{
            ...
            <button onClick={async () => {
                ...
                setPlacing(true);
                try {
                  // Submit the cart to the kitchen queue (lands as pending_approval
                  // for the Captain). window.AkanAPI is injected by akan-api.js.
                  await window.AkanAPI.placeOrder(items.map(({ item, qty }) => ({ id: item.id, qty })));
                  onPlaced(total);
                } catch (err) {
                  setPlacing(false);
                  (window.AkanAPI && window.AkanAPI.toast)
                    ? window.AkanAPI.toast(err && err.message ? err.message : 'Could not place your order — please try again.')
      ...
      <div style={{
        ...
        <div style={{ padding: '16px 18px 0', display: 'flex', flexDirection: 'column', gap: 8 }}>
          {actions.map(a => {
            ...
            return (
              <button key={a.id} onClick={() => { setRequested(a.id); if (window.AkanAPI) window.AkanAPI.serviceRequest(a.id); }} style={{
