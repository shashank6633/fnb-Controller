...
export function listDiscountRequests(db: Database.Database, whereSql: string, params: unknown[]) {
  ...
  // ₹ impact if approved — same math the sync route writes (subtotal × pct / 100).
  return rows.map((r) => ({ ...r, impact_amount: round2((Number(r.order_subtotal) || 0) * (Number(r.requested_pct) || 0) / 100) }));
}
