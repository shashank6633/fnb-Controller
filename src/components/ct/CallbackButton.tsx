...
export default function CallbackButton({ phone, guestId, recoveryId, guestName, label = 'Call Back', className, onLogged }: Props) {
  ...
  const openSheet = useCallback((elapsedSec: number) => {
    setMins(Math.floor(elapsedSec / 60));
    setSecs(elapsedSec % 60);
    setConnected(elapsedSec > 0);
    setOutcome('');
    setNote('');
    setError('');
    setOpen(true);
  ...
  const save = async () => {
    ...
    try {
      ...
      const r = await api('/api/crm-calls/calls/log-callback', {
        method: 'POST',
        body: { phone, guest_id: guestId, recovery_id: recoveryId, duration_sec, connected, outcome, note },
      });
