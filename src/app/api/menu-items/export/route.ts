...
const HEADERS = [
  'Item ID', 'Category', 'Name', 'Selling Price', 'Listing Price', 'Master Status',
  'Item Type', 'Tax Value', 'Item Code', 'Station', 'Dietary Tag', 'POS ID',
];

// Item ID is the platform's stable identifier: keep it untouched when editing
// and the import updates THAT item even if you rename it. Blank ID = new item.
  ...
  try {
    ...
    if (sample) {
      ...
      const items = db.prepare(`
        SELECT id, category, name, selling_price, listing_price, is_active,
               item_type, tax_value, item_code, station, dietary_tag, pos_id
        FROM menu_items
        ORDER BY category COLLATE NOCASE, name COLLATE NOCASE
      `).all() as any[];
      rows = items.map((i) => [
        i.id, i.category || '', i.name, i.selling_price ?? 0, i.listing_price ?? 0,
