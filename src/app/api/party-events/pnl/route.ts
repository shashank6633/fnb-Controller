...
function loadUpcomingPartiesCache(): { party_unique_id?: string; fp_id?: string; event_name?: string; event_date?: string; guest_name?: string; company?: string; contact_person?: string; pax_expected?: number; min_guarantee?: number }[] {
  try {
    const db = getDb();
...
function pnlFor(db: ReturnType<typeof getDb>, bookings: Map<string, number>,
  ...
  // Revenue from bookings (matched by party_unique_id)
  let revenue = 0;
  if (key.party_unique_id && bookings.has(key.party_unique_id)) {
    revenue = bookings.get(key.party_unique_id) || 0;
  }
...
export async function GET(request: Request) {
  try {
    ...
    if (party_unique_id || (event_name && event_date)) {
      const pnl = pnlFor(db, bookings, { party_unique_id, event_name, event_date });
      return Response.json({ pnl });
    }

    // Bulk: every cached party that has either booking revenue OR food/liquor cost data
