...
import {
  ...
  Send, RotateCcw, ChevronRight, ChevronDown, History, User as UserIcon,
} from 'lucide-react';
            ...
            <tbody>
              {req.items.map(line => (
                <LineRow key={line.id} line={line} req={req}
                         ...
                         onUndo={props.onUndo} onShowHistory={props.onShowHistory} />
              ))}
            </tbody>
          </table>
  ...
  return (
    <tr className={`border-t border-[#E8D5C4]/50 ${rowTone}`}>
      <td className="py-1.5 px-2 align-top">
        <div className="font-medium text-[#2D1B0E]">{line.material_name}</div>
        ...
        {line.deferred_until && (
          <div className="text-[10px] text-blue-700 mt-0.5 flex items-center gap-1">
