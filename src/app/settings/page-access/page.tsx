...
export default function PageAccessSettingsPage() {
  ...
  const [expanded, setExpanded] = useState<Set<string>>(new Set(['Parties', 'Store'])); // default-expand likely-used sections
                      ...
                      <div key={section.label}>
                        ...
                        {isExpanded && (
                          <div className="px-4 py-2 grid grid-cols-2 md:grid-cols-3 gap-1.5 text-xs">
                            {section.pages.map(p => (
                              <label key={p.path} className="flex items-center gap-1.5 cursor-pointer hover:bg-[#FFF8F0] px-1 py-0.5 rounded">
