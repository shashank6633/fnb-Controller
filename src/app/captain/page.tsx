...
export default function CaptainHome() {
  ...
  const open = useMemo(() => tables.filter((t) => t.open_order_id)
    .sort((a, b) => (b.open_order_total || 0) - (a.open_order_total || 0)), [tables]);
    ...
    <div className="p-4 sm:p-6 pb-24 lg:pb-6 max-w-4xl">
      ...
      {loading ? (
        ...
        <div className="text-center py-16 px-6 bg-white border border-[#E8D5C4] rounded-2xl">
          ...
          <p className="font-semibold text-[#2D1B0E]">No open tables right now</p>
          <p className="text-sm text-[#8B7355] mt-1">Pick a table from the sidebar (or the <b>Tables</b> button on phones) to start an order, or begin a Takeaway.</p>
        </div>
      ) : (
